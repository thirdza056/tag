...
print ("====Saber登入成功====")
kt = LINE("xyukino0042@gmail.com", "e27676271")
...
def helpmessage():
    ...
    return helpMessage
def ban(mid, to):
    if mid not in settings['admin'] or mid not in settings['bot']:
        settings['blacklist'][mid] = True
        cl.sendMessage(to, "成功新增黑單")
        cl.sendContact(mid)
        cl.sendMessage(mid, "[警告]\n你因為違反yukino使用規則或只是作者看你不爽而已wwww,因此你已經被列入黑名單將不能正常使用yukino了!!!!!")
        backupData()
    else:
        del settings['admin']
        settings['blacklist'][mid] = True
        backupData()
        cl.sendMessage(to, "你因為嘗試黑單作者或是本機之附屬機器,因此你已經被刪除管理員權限\n所以......掰掰囉")
        try:
            ...
            if msg.contentType == 0:
                ...
                if settings['limit'][sender]['text']['react'] == True:
                    ...
                    if text.lower() == 'help':
                        ...
                        cl.sendMessage(to, "人々は痛みを伴う記憶を封印できますか.")
                        kl.sendMessage(to, "私はあなたの問題を是正して、私に感謝しましょう")
                        sa.sendMessage(to, "いいえ、私はその名前を呼ぶ人は知らない。s")
                        sb.sendMessage(to, "いざ、死力を盡くして來るがいい。この剣にかけて、貴様等の挑戦に応えよう－－！")
                        kt.sendMessage(to, "初音ミク......")
